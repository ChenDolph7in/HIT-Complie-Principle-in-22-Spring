...
int checkType(struct type *t1, struct type *t2)
{
    ...
    else
    {
        ...
        {
        case BASIC:
            return !strcmp(t1->UNION.TOKEN, t2->UNION.TOKEN);
        case ARRAY:
            return checkType(t1->UNION.ARRAY.elemType, t2->UNION.ARRAY.elemType) && (t1->UNION.ARRAY.size == t2->UNION.ARRAY.size);
        case STRUCTURE:
            // printf("t1%s\n",t1->UNION.STRUCT.structName);
...
struct type *StructSpecifier(struct node *node)
{
    ...
    if (strcmp(temp->token, "Tag"))
    {
        ...
        if (!strcmp(temp->token, "OptTag"))
        {
            ...
            myType->UNION.STRUCT.structName = temp->left_sons->type;
            temp = temp->siblings;
        }
